#!/usr/bin/env python3
"""
CipherMail — All Functions Extracted
...
# ═══════════════════════════════════════════════════════════════════
#  SECTION A — TERMINAL / DISPLAY HELPERS
# ═══════════════════════════════════════════════════════════════════
...
def info(msg):
    ...
    print(f"  {c('ℹ', C.CYAN)}  {msg}")


def bold(msg):
    """Print a bold message with a preceding newline."""
    print(f"\n{c(msg, C.BOLD)}")
...
def banner():
    """Print the CipherMail ASCII art banner."""
    print(c("""
╔═══════════════════════════════════════════════════════════════╗
║     ██████╗██╗██████╗ ██╗  ██╗███████╗██████╗               ║
║    ██╔════╝██║██╔══██╗██║  ██║██╔════╝██╔══██╗              ║
...
def divider_title(title):
    """Print a section title with double-line separators."""
    print()
    sep('═')
...
def pem_snippet(pem_bytes: bytes, lines: int = 4) -> str:
    """Return the first and last N lines of a PEM block."""
...
def fingerprint(public_key) -> str:
    """Return the SHA-256 fingerprint of a public key as a hex string."""
...
class AppState:
    ...
    def has_key(self) -> bool:
        """Return True if both private and public keys are loaded."""
...
# ═══════════════════════════════════════════════════════════════════
#  SECTION C — KEY MANAGEMENT
# ═══════════════════════════════════════════════════════════════════

def cmd_generate_key():
    """Interactive: Generate an RSA-2048 or RSA-4096 key pair."""
    ...
    STATE.key_store.append({"owner": owner, "algo": f"RSA-{bits}", "fingerprint": fp, "created": now_iso()})
    STATE.stats["keys"] += 1
...
def cmd_show_key_store():
    """Display all key pairs currently stored in the session key store."""
...
def cmd_load_key():
    """Load an existing PEM key pair from disk into the session state."""
...
def cmd_issue_cert():
    """Issue a self-signed X.509 certificate for the active key pair."""
    ...
    cert = (
        ...
        .add_extension(x509.SubjectKeyIdentifier.from_public_key(STATE.public_key), critical=False)
...
def cmd_validate_cert():
    """Validate a certificate by checking its expiry, validity window, and revocation status."""
...
def cmd_revoke_cert():
    """Add the active certificate's serial number to the in-memory CRL."""
    ...
    if confirm == "yes":
        ...
        ok(f"Certificate {c(serial_hex, C.RED)} added to CRL.")
...
# ═══════════════════════════════════════════════════════════════════
#  SECTION E — SIGNING & VERIFICATION
# ═══════════════════════════════════════════════════════════════════

def _rsa_sign(message: bytes) -> bytes:
    """Sign a message with the active private key using RSA-PSS-SHA256."""
...
def _rsa_verify(public_key, message: bytes, signature: bytes) -> bool:
    """Verify an RSA-PSS-SHA256 signature. Returns True if valid."""
    try:
        public_key.verify(
            signature, message,
            padding.PSS(mgf=padding.MGF1(hashes.SHA256()), salt_length=padding.PSS.MAX_LENGTH),
...
def cmd_sign():
    """Interactive: Sign a typed message or file with the active private key."""
...
def cmd_verify():
    """Interactive: Verify a signed JSON package from file, clipboard, or memory."""
...
def cmd_multi_party_sign():
    """Demo: Simulate 3 authorized signers and 1 attacker signing a document."""
    ...
    signers = [
        ...
        ("Eve Mallory <eve@attacker.com>", False),
    ]
    results = []
...
# ═══════════════════════════════════════════════════════════════════
#  SECTION F — HYBRID ENCRYPTION / DECRYPTION
# ═══════════════════════════════════════════════════════════════════

def cmd_encrypt():
    """Interactive: Encrypt a message or file using RSA-OAEP + AES-256-GCM."""
...
def cmd_decrypt():
    """Interactive: Decrypt an RSA-OAEP + AES-256-GCM package."""
...
# ═══════════════════════════════════════════════════════════════════
#  SECTION G — EMAIL FORENSICS
# ═══════════════════════════════════════════════════════════════════

def _parse_headers(raw: str) -> dict:
    """Parse raw email text into a lowercase header dict."""
    headers = {}
    current_key = ""
...
def cmd_email_forensics():
    """Interactive: Analyse email headers for spoofing and phishing indicators."""
...
"""
    if choice == "3":   raw = SPOOFED_EMAIL
    elif choice == "4": raw = LEGIT_EMAIL
    elif choice == "2":
        path = input(c("  File path: ", C.CYAN)).strip()
...
# ═══════════════════════════════════════════════════════════════════
#  SECTION H — ATTACK SIMULATIONS
# ═══════════════════════════════════════════════════════════════════

def cmd_attack_simulations():
    """Interactive: Choose and run individual or all attack simulations."""
...
def _sim_mitm():
    """Simulate MITM message tampering and show that RSA signatures detect it."""
...
def _sim_replay():
    """Simulate a replay attack and show that timestamp validation rejects it."""
...
def _sim_weak_key():
    """Display a table of RSA key sizes and their security classification."""
    ...
    for algo, bits, status, col, note in keys:
        enforced = bits < 2048
        tag = c("[BLOCKED by policy]", C.RED) if enforced else c("[ALLOWED]", C.GREEN)
...
def _sim_cert_spoof():
    """Generate a forged certificate and verify it is not in the trust store."""
...
def cmd_audit_log():
    """Display the full hash-chained audit log with integrity hash."""
...
# ═══════════════════════════════════════════════════════════════════
#  SECTION J — USE CASE DEMOS
# ═══════════════════════════════════════════════════════════════════

def _ensure_keys():
    """Auto-generate an RSA-2048 demo key pair if none is currently loaded."""
    if not STATE.has_key():
...
def cmd_use_cases():
    """Interactive: Launch one of three real-world use-case demos."""
...
def _uc_legal():
    """Demo: Sign and verify a legal contract document."""
...
def _uc_email_auth():
    """Demo: Sign an email and show that a spoofed version fails verification."""
...
def _uc_medical():
    """Demo: Encrypt, sign, and decrypt a medical record with full audit logging."""
...
# ═══════════════════════════════════════════════════════════════════
#  SECTION K — STATISTICS & MENU
# ═══════════════════════════════════════════════════════════════════

def cmd_stats():
    """Display session statistics: key count, signatures, encryptions, etc."""
...
def run_interactive():
    """Run the full interactive CLI menu loop."""
    banner()
...
# ═══════════════════════════════════════════════════════════════════
#  SECTION L — CLI ARG MODE
# ═══════════════════════════════════════════════════════════════════

def cli_mode():
    """Entry point: parse CLI args or fall back to interactive menu."""
